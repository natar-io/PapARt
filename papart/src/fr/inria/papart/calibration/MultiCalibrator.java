...
public class MultiCalibrator extends PaperTouchScreen {
  ...
  // -> When paperScreen stops moving (< 1cm?), capture all images for blinking
  // analysis.
  // Visual indication that the movement stopped (OK stay still for x seconds...)
  // Compute center position from Markers + center from blinking points if all
  // found.
  ...
  // 6. Compute color histograms, take the most commons, compute H,S,B + R,G,B
  // means + stdev.
  // 7. Check the colors for color tracking across 4-6 screenshots. save the
  // colors.
  ...
  private void initProjectorAsCamera() {
    ...
    // projectorTracker = DetectedMarker.createDetector(projector.getWidth(),
    // projector.getHeight());
    // }
  ...
  public void drawOnPaper() {
    ...
    if (active) {
      ...
      // PVector pxCam = cameraTracking.getProjectiveDevice().worldToPixelCoord(pos3D,
      // false);
  ...
  void savePicture() {
    ...
    if (!seeThrough && useDepth) {
      ...
      // Save also the 3D point from depth cam hopefully there are no fingers or
      // objects.
      ...
      // System.out.println("DepthPoints: " + ((DepthTouchInput)
      // touchInput).getDepthAnalysis().getDepthPoints());
  ...
  void calibrate() {
    ...
    if (useNectar) {
      ...
      for (int i = 0; i < nbScreenPoints; i++) {
        ...
        // opencv_imgcodecs.cvSaveImage("/home/ditrop/tmp/proj-" + i + ".bmp",
        // projectorAsCamera.getIplImage());
    ...
    if (useDepth) {
      ...
      for (int i = 0; i < nbScreenPoints; i++) {
        ...
        PlaneCalibration planeCalib = PlaneCalibration.CreatePlaneCalibrationFrom(paperViewedByDepth,
            // app.getLocation(),
            new PVector(100, 100));
      ...
      HomographyCalibration homography = ExtrinsicCalibrator.computeScreenPaperIntersection(projector, planeCalibCam,
          depthCamExtrinsics);
    ...
    if (multiCalibrator.getDisplay() instanceof ProjectorDisplay) {
      ...
      if (multiCalibrator.getDisplay() instanceof ARDisplay) {
        ...
        if (multiCalibrator.getCameraTracking() != null && img != null) {
          ...
          // ((PGraphicsOpenGL) (parent.g)).image(camera.getPImage(), 0, 0, frameWidth,
          // frameHeight);
        }
  ...
  public static void drawHints(PGraphicsOpenGL g,
    ...
    // float freq = 0.2f + 2f * ((float) multiCalibrator.pressedAmt / (float)
    // multiCalibrator.maxPressedAmt);
    // float freq = 0.5f;
    ...
    // float v = (PApplet.sin((parent.millis() / 1000f) * PConstants.TWO_PI * freq)
    // + 1f) / 2f;
    // g.fill(255 * v);
    ...
    // g.rect(-50, -100, 150, 80);
    // float d =
    // multiCalibrator.getMarkerBoard().lastMovementDistance(multiCalibrator.getCameraTracking());
    // g.text(d, 100, 100);
