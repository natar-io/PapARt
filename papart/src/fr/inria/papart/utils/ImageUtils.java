...
public class ImageUtils {
    ...
    if (img.nChannels() == 3) {
      ...
      if (RGB) {
        for (int i = 0; i < img.width() * img.height(); i++) {
          int offset = i * 3;
          // ret.pixels[i] = applet.color(buff.get(offset + 0) & 0xff, buff.get(offset +
          // 1) & 0xFF, buff.get(offset + 2) & 0xff);
          ret.pixels[i] = (buff.get(offset) & 255) << 16 | (buff.get(offset + 1) & 255) << 8
              | (buff.get(offset + 2) & 255);
        }
      } else {
        for (int i = 0; i < img.width() * img.height(); i++) {
          int offset = i * 3;
          // ret.pixels[i] = applet.color(buff.get(offset + 0) & 0xff, buff.get(offset +
          // 1) & 0xFF, buff.get(offset + 2) & 0xff);
          ret.pixels[i] = (buff.get(offset + 2) & 255) << 16 | (buff.get(offset + 1) & 255) << 8
              | (buff.get(offset) & 255);
        }
      }
  ...
  public static void remapImageIpl(Mat homography, IplImage imgIn, IplImage imgOut) {
    // System.out.println("cam: " + imgIn.width() + " " + imgIn.height() + " " +
    // imgIn.depth());
    // System.out.println("extr: " + imgOut.width() + " " + imgOut.height() + " " +
    // imgOut.depth());
  ...
  public static IplImage createImageFrom(PImage in) {
    ...
    // System.out.println("inputImage to create an IPL:" + in.width + " " +
    // in.height + " " + in.format);
  ...
  // Texture tex = null;
  //
  // // We suppose... Depth = 3 : BGR and Depth = 4 : RGBA (even though it is
  // written ARGB for Processing...)
  // if (img.nChannels() == 3) {
